...
iqDataTx=0.3*m.*exp(i*2*pi*t/8); % amplitude modulate offset ssb carrier
%iqDataTx=0.3*m; % amplitude modulate carrier - does not work due to RX DC cancellation!
%
%   R A D I O   S E T U P
%
if useLimeSDR==true 
...
for cint=1:itt  
 ...
 if useLimeSDR==true 
  ...
  ask=abs(iqDataRx); % basic FM demodulator
  subplot(2,2,1);
  ...
  plot(ask,'b-');
  grid on;
  title('Demodulated ASK Waveform');
  xlabel('Time Points');
  ylabel('Level');
 
  subplot(2,2,3);  % basic windowed water fall type graph. 
